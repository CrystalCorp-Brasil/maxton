...
@endif
                                ...
                                <div class="d-none flex-sm-fill d-sm-flex align-items-sm-center justify-content-sm-between">
                                    <div class="col-8 mt-2">
                                        <p class="small text-orange">{!! __('Showing') !!} <span class="fw-semibold text-success">{{ $paginator->firstItem() }}</span> {!! __('to') !!} <span class="fw-semibold text-success">{{ $paginator->lastItem() }}</span> {!! __('of') !!} <span class="fw-semibold text-success">{{ $paginator->total() }}</span> {!! __('results') !!}</p>
                                    </div>
                                    <div class="col-4">
                                        <ul class="pagination">
@if ($paginator->onFirstPage())
