                                        ...
                                        <div class="col-md-3 col-lg-12 mt-6 mt-md-0 mt-lg-6">
                                            ...
                                            <div class="overflow-hidden">
                                                ...
                                                <div class="reaction-buttons d-flex gap-3 align-items-center mt-3" data-type="char" data-slug="{{ $char->slug }}">
                                                    <button type="button" class="btn btn-like btn-outline-success" data-type="char" data-slug="{{ $char->slug }}" data-action="like" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Curtir">
                                                        <i class="bi bi-hand-thumbs-up"></i><span data-count-type="like" data-slug="{{ $char->slug }}">{{ $likesCount ?? 0 }}</span>
                                                    </button>
                                                    <button type="button" class="btn btn-dislike btn-outline-danger" data-type="char" data-slug="{{ $char->slug }}" data-action="dislike" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Não curtir">
                                                        <i class="bi bi-hand-thumbs-down"></i>
                                                        <span data-count-type="dislike" data-slug="{{ $char->slug }}">{{ $dislikesCount ?? 0 }}</span>
                                                    </button>
                                                </div>
                                                <div class="card mt-3">
...
@section('scripts')
        <script>
            window.CONTENT_TYPE = 'char';
            window.CONTENT_SLUG = @json($char->slug);
            window.AUTH_ID = {{ auth()->id() ?? 'null' }};
        </script>
@endsection
